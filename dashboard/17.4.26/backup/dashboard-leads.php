...
<section class="content_body">
    <div class="filter_wrapper">
        <form method="GET" id="lead-filter-form" action="">
            ...
            <a href="<?php echo get_permalink(); ?>" class="glb_btn reset">Reset</a>
        </form>
    </div>
                ...
                if ($results) {
                    ...
                    foreach ($results as $row) {
                        ...
                        <tr data-radar-id="<?php echo esc_attr($row->radar_id); ?>">
                            ...
                            <td><?php echo date_i18n(get_option('date_format'), strtotime($row->fetched_at)); ?></td>
                            <td><?php echo esc_html($contact_name); ?></td>
                            <td>
                                <div class="copy_wrapper">
                                    <span class="copy_text"><?php echo esc_html($email); ?></span>
                                    <?php if ($email !== 'NA'): ?>
                                        <button class="copy_btn" data-copy="<?php echo esc_attr($email); ?>"><i class="fa-solid fa-copy"></i></button>
                                    <?php endif; ?>
                                </div>
                            </td>
                            <td>
                                <div class="copy_wrapper">
                                    ...
                                    <?php if ($phone !== 'NA'): ?>
                                        <button class="copy_btn" data-copy="<?php echo esc_attr($phone); ?>"><i class="fa-solid fa-copy"></i></button>
                                    <?php endif; ?>
                                </div>
                            </td>
                            <td>
                                <div class="action_btns">
                                    <button class="btn_view" data-radar-id="<?php echo esc_attr($row->radar_id); ?>">View</button>
                                    <button class="btn_edit convert-customer" data-radar-id="<?php echo esc_attr($row->radar_id); ?>">Convert Customer</button>
                                </div>
                            </td>
                        </tr>
                        <?php
            ...
            success: function(response) {
                if (response && response.success) {
                    ...
                    $('#modal-property-info').html(
                        '<p><strong>Address:</strong> ' + (data.address || 'NA') + '<br>' +
                        '<strong>City:</strong> ' + (data.city || 'NA') + '<br>' +
                        '<strong>Zip:</strong> ' + (data.zip_five || 'NA') + '<br>' +
                        '<strong>State:</strong> ' + (data.state || 'NA') + '<br>' +
                        '<strong>Property Type:</strong> ' + (data.property_type || 'NA') + '<br>' +
                        '<strong>SqFt:</strong> ' + (data.sqft || 'NA') + '<br>' +
                        '<strong>Beds:</strong> ' + (data.beds || 'NA') + '<br>' +
                        '<strong>Baths:</strong> ' + (data.baths || 'NA') + '</p>'
                    );
                    var ownersHtml = '<table class="wp-list-table widefat"><thead><tr><th>Name</th><th>Primary</th><th>Phone</th><th>Email</th></tr></thead><tbody>';
                    if (data.owners && data.owners.length) {
                        $.each(data.owners, function(i, owner) {
                            ...
                            ownersHtml += '<tr>' +
                                ...
                                '<td>' + isPrimary + '</td>' +
                                '<td>' + (owner.phone || 'NA') + '</td>' +
                                '<td>' + (owner.email || 'NA') + '</td>' +
                            '</tr>';
                        });
                    } else {
